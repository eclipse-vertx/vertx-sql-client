...
You can change the server type of the embedded database by passing a case-insensitive `embedded.database.server` option like this:

```
> mvn test -Dembedded.database.server=MySQL
```
...
You can also change the version of the embedded database by passing a `embedded.database.version` option like this:
...
The MariaDB database for testing is using TestContainers so the version should be a Docker tag like this:
