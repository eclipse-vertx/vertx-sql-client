...
You can also manage the lifecycle of prepared statements manually by creating a `PreparedQuery` object via `SqlConnection#prepare` interface, or even use the https://dev.mysql.com/doc/refman/8.0/en/sql-prepared-statements.html[SQL syntax prepared statement].

=== demystifying prepared batch

There is time when you want to batch insert data into the database, you can use `SqlClient#preparedBatch` which provides a simple API to handle this.
