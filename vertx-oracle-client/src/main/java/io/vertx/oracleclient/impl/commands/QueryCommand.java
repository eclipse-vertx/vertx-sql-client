...
public abstract class QueryCommand<C, R> extends AbstractCommand<Boolean> {
  ...
  public Future<Boolean> execute(OracleConnection conn, ContextInternal context) {
    ...
    return psFuture.compose(ps -> withPreparedStatement(ps, context, returnAutoGeneratedKeys));
  }
  ...
  private Future<Boolean> withPreparedStatement(OraclePreparedStatement ps, ContextInternal context, boolean returnAutoGeneratedKeys) {
    return doExecute(ps, context, returnAutoGeneratedKeys).transform(ar -> {
      Future<Void> close;
      if (closeStatementAfterExecute()) {
        close = executeBlocking(context, () -> closeQuietly(ps));
      } else {
  ...
  protected abstract Future<Boolean> doExecute(OraclePreparedStatement ps, ContextInternal context, boolean returnAutoGeneratedKeys);
