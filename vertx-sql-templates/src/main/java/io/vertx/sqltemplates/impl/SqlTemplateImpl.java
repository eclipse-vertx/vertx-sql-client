package io.vertx.sqltemplates.impl;
...
public class SqlTemplateImpl<T, R> implements io.vertx.sqltemplates.SqlTemplate<T, R> {
  ...
  @Override
  public <T1> io.vertx.sqltemplates.SqlTemplate<T1, R> mapFrom(Function<T1, Map<String, Object>> mapper) {
    return new SqlTemplateImpl<>(client, sqlTemplate, queryMapper, mapper);
  }

  @Override
  public <T1> io.vertx.sqltemplates.SqlTemplate<T1, R> mapFrom(Class<T1> type) {
    return mapFrom(params -> JsonObject.mapFrom(params).getMap());
  }

  @Override
  public <U> io.vertx.sqltemplates.SqlTemplate<T, SqlResult<U>> collecting(Collector<Row, ?, U> collector) {
    return new SqlTemplateImpl<>(client, sqlTemplate, query -> query.collecting(collector), paramsMapper);
  }

  @Override
  public <U> io.vertx.sqltemplates.SqlTemplate<T, RowSet<U>> mapTo(Class<U> type) {
    return mapTo(row -> {
  ...
  @Override
  public <U> io.vertx.sqltemplates.SqlTemplate<T, RowSet<U>> mapTo(Function<Row, U> mapper) {
