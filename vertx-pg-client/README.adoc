...
You can change the version of the Postgres server by passing a property `testing.postgres.database.version` like this:

```
> mvn test -Dtesting.postgres.database.version=9.6
```

The database version property would be used as the tag of the Postgres Docker container, if the database version is not specified, then `10.10` will be used by default for Postgres
...
- the `UnixDomainSocketTest`

You need to add some properties for testing:

```
> mvn test -Dconnection.uri=postgres://$username:$password@$host:$port/$database -Dtls.connection.uri=postgres://$username:$password@$host:$port/$database -Dunix.socket.connection.uri=postgres://$username:$password@$host:$port/$database
```
...
- unix.socket.connection.uri(optional): test Unix domain socket feature with a specified database, domain socket tests will be skipped if this property is not specified
(Note: Make sure you can access the unix domain socket with the directory under your host machine, Postgres unix socket file is named `.s.PGSQL.nnnn` and `nnnn` is the server's port number, you will need to set the port option when you publish your Postgres container port other than 5432 in your host but Postgres may actually listen on a different port in the container)
...
```
> mvn test -Dconnection.uri=postgres://$username:$password@$host:$port/$database -Dtls.connection.uri=postgres://$username:$password@$host:$port/$database
```
