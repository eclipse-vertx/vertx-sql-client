...
public class SharedPoolTest extends PgTestBase {
  ...
  public void testUseSamePool(TestContext ctx) {
    ...
    vertx.deployVerticle(() -> new AbstractVerticle() {
      ...
      @Override
      public void start() {
        pool = PgBuilder.pool().connectingTo(options).config(new PoolOptions().setMaxSize(maxSize).setShared(true)).using(vertx).build();
        pool
      ...
      public void start() {
        ...
        vertx.deployVerticle(() -> new AbstractVerticle() {
          ...
          public void start(Promise<Void> startPromise) {
            Pool pool = PgBuilder.pool().connectingTo(options).config(new PoolOptions().setMaxSize(maxSize).setShared(true)).using(vertx).build();
