...
conn.prepare("SELECT * FROM USERS WHERE user_id=$1") { |ar1_err,ar1|
  ...
  if (ar1_err == nil)
    ...
    query = preparedStatement.query(VertxPgClient::Tuple.of("julien"))
...
----
require 'vertx-pg-client/tuple'
conn.prepare("UPDATE USERS SET name=$1 WHERE id=$2") { |ar1_err,ar1|

  if (ar1_err == nil)
    preparedStatement = ar1

    # Create an update : bind parameters
    update = preparedStatement.query(VertxPgClient::Tuple.of(2, "EMAD ALBLUESHI"))
    ...
    # Or fluently
    preparedStatement.query(VertxPgClient::Tuple.of(1, "JULIEN VIET")).execute() { |res_err,res|
      if (res_err == nil)
        # Process results
        result = res
      else
