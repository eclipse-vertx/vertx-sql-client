...
The `link:../../jsdoc/module-vertx-pg-client-js_json-Json.html[Json]` Java type is used to represent the Postgres `JSON` and `JSONB` type.

The main reason of this type is handling `null` JSON values.

[source,js]
----
var Json = require("vertx-pg-client-js/json");
var Tuple = require("vertx-pg-client-js/tuple");
