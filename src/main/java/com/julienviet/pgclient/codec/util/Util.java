...
public class Util {
  ...
  public static void handleRow(Column[] columns, DataRow dataRow, JsonArray row) {
  ...
  private static void handleText(DataType type, byte[] data, JsonArray row) {
    if(data == null) {
      row.addNull();
      return;
    }
  ...
  private static void handleBinary(DataType dataType, byte[] d, JsonArray row) {
  }
