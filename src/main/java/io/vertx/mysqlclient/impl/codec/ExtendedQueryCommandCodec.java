...
public class ExtendedQueryCommandCodec<R> extends QueryCommandBaseCodec<R, ExtendedQueryCommand<R>> {
  ...
  void encodePayload(MySQLEncoder encoder) {
    ...
    MySQLPreparedStatement ps = (MySQLPreparedStatement) cmd.preparedStatement();
    writeExecuteMessage(encoder, ps.statementId, ps.paramDesc.paramDefinitions(), sendType, cmd.params());
  }

  private void writeExecuteMessage(MySQLEncoder encoder, long statementId, ColumnDefinition[] paramsColumnDefinitions, byte sendType, Tuple params) {
