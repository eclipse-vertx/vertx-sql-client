...
class SimpleQueryCommandCodec<T> extends CommandCodec<Boolean, SimpleQueryCommand<T>> {
...
//          handleErrorPacketPayload(ctx, payload);
      ...
      case HANDLING_COLUMN_DEFINITION:
        System.out.println("handling def");
...
//          // all column definitions have been handled, switch to row data handling
          ...
          decoder = new RowResultDecoder<>(cmd.collector(), cmd.isSingleton(), new ColumnMetadata(columnDefinitions, DataFormat.TEXT));
        }
        break;
      case HANDLING_ROW_DATA_OR_END_PACKET:
        ...
        // we need check this is not a row data by checking packet length < 0xFFFFFF
          ...
          // accept a row data
          System.out.println("decoding row");
  ...
  private void handleResultsetDecodingCompleted(SimpleQueryCommand cmd) {
  ...
  private ColumnDefinition decodeColumnDefinitionPacketPayload(ByteBuf payload) {
